...
/**
 * SegmentedProgressBar with dividers, taken from internet, modified to show progress
 */
public class SegmentedProgressBar extends View {

    private static final String TAG = "SegmentedProgressBar";
    private RectF bgRect;
    private final Paint progressBarBackgroundPaint = new Paint();
            ...
            viewTreeObserver.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    if (getWidth() > 0) {
                        ...
                        segmentWidth = (float) progressBarWidth / divisions;
                        segmentIncrementSlot = segmentWidth /(float) segmentDivisions;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (bgRect != null) {
            ...
            int enabledDivision = enabledDivisions.size();//current segment
            float left = enabledDivision > 0 ? dividerPositions.get(enabledDivision - 1) + dividerWidth : 0;
            //  float right = dividerPositions.get(enabledDivision);

            RectF rect = new RectF(left, 0, left + progressX, getHeight());
            canvas.drawRect(rect, progressBarPaint);
        }
    }
    ...
    float segmentIncrementSlot = 0;
    public int segmentDivisions = 10;
    ...
    public void refresh() {
        if (progressX + segmentIncrementSlot < segmentWidth) {
            progressX = progressX + segmentIncrementSlot;
        } else {
            progressX = 0;
        }
        invalidate();
